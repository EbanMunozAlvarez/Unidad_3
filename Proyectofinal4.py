...
def abrir_reportes():
    ...
    total_label = tk.Label(ventana, text="Total de ventas: $0.00", font=("Arial", 12), bg="#f2f2f2")
            ...
            for linea in archivo:
                if linea.strip():
                    ...
                    if len(datos) == 4:
                        ...
                        # Sumar el total de cada venta (suponiendo que el total es un número)
                        try:
                            total_ventas += float(datos[3])  # Datos[3] es el total de la venta
                        except ValueError:
                            pass  # En caso de que no se pueda convertir el total a float, se ignora

        # Actualizar el label con el total de ventas
        total_label.config(text=f"Total de ventas: ${total_ventas:.2f}")
